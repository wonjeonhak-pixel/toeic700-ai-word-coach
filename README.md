# QuietStep — TOEIC700 AI英単語コーチ

**「なぜ間違えたか」が分かり、TOEIC700への前進を実感できる、社会人向けAI英単語コーチ。**

**QuietStep** は、通勤30分・昼休み15分で1セッションが完結する、10問 × AIフィードバック型の英単語アプリです。Next.js (App Router) + TypeScript + Anthropic Claude API で実装しています。
...
**QuietStep** は、強いゲーミフィケーションや連続学習の圧ではなく、「5分で終われる」「また戻ってきやすい」**quiet UX** を重視して設計しました。社会人が通勤や昼休みに無理なく開けるよう、10問完結・短いAIフィードバック・静かな前進感を重視しています。streakやバッジ、派手な達成演出を意図的に置かず、結果画面の「次回の重点単語」で次に開いたときの戻り先だけを静かに示します。

---

## 学習特性 — 速度と例文音声

TOEIC700レベルは、Reading / Listening それぞれ 300〜400 点程度が目標になります。このレベルでは単語を「知っている」だけでは足りず、見聞きした瞬間に意味が浮かぶ「**瞬時理解**」の速さが要求されます。「思い出す」では遅い、というのが体感としての境目です。

また、実際の業務シーンでは単語単体を耳にする場面は少なく、語彙はほぼ常に **文の中で音声として** 入ってきます。そのため本アプリは、毎問の AI フィードバックに `business_example`（実務文脈の英語例文）と 🔊 音声再生ボタンを必ずセットで返す設計にしました。意味の暗記だけで終わらず、「文として聞いたときに意味が浮かぶ感覚」を1日3分のセッションでも少しずつ積み上げられます。結果としてリスニング側の地力にも繋がる構成です。

quiet UX の方針通り、速読・速聴を強要するUIや煽りは入れていません。例文＋音声を「毎回ある自然な学習素材」として静かに置き、開くたびに少しずつ瞬時理解の入口に寄せていきます。

---

## AI駆動開発プロセスの見える化

実装に先立ち、戦略・要求仕様・実装タスクを `specs/` 配下に整理しました。

- [`specs/strategy.md`](./specs/strategy.md) — アプリコンセプト・ターゲット・解決する課題・提供価値・設計思想・非ゴール
- [`specs/prd.md`](./specs/prd.md) — 1プレイ10問 / mistake_type 5択 / learner_type / 音声再生 / quiet UX 等を含む13項目の要求仕様と受け入れ基準
- [`specs/backlog.md`](./specs/backlog.md) — AI-DLC 的に小さな単位へ分解した実装タスク一覧（すべて完了済み・チェックボックス形式）

Claude Code にはこの仕様群をもとに実装と改善を依頼しました。最初に戦略・PRD・バックログを言語化してから AI に渡すことで、生成された JSON スキーマ・プロンプトルール・UI 文言の方向性を毎回ゼロから議論する必要がなくなり、AIとの認識ズレを大きく減らせています。プロンプト調整時も `specs/prd.md` の受け入れ基準を判定軸にできるため、出力品質の回帰検知が安定しました。
...
1. **ホーム** (`/`) — 「現在の重点：TOEIC600→700レベル」表示、「今日の10問を始める」ボタン
2. **クイズ** (`/quiz`) — 10問のランダム出題＋毎問のAIフィードバック。例文には🔊「聞く」ボタン
3. **結果** (`/result`) — 正答数 + AI総括（弱点 / 成長 / 次にやること / TOEIC700メッセージ）+ 10問振り返り

スマホ縦長UIを想定したライト基調（クリーム背景 × accent ブルー）です（max-width 480px）。

---

## スクリーンショット

QuietStep の **ホーム → クイズ → 結果（3 分割）** を撮影。クリーム基調のライトテーマ × 静かな視線誘導の温度感を伝えます。

| ホーム | クイズ（回答前） | 不正解時フィードバック |
|---|---|---|
| ![ホーム](./docs/screenshots/01_home.png) | ![クイズ](./docs/screenshots/02_quiz.png) | ![フィードバック](./docs/screenshots/03_feedback.png) |

| 結果（総括・learner_type） | 次回の重点単語 | 結果（最下部 signature） |
|---|---|---|
| ![結果総括](./docs/screenshots/04_result_summary.png) | ![次回の重点単語](./docs/screenshots/05_focus_words.png) | ![フッター signature](./docs/screenshots/06_result_footer.png) |

各画像の対応する UI 要素は [`docs/screenshots/README.md`](./docs/screenshots/README.md) を参照。

---

## Demo Video（optional）

[`docs/demo/quietstep_demo.mp4`](./docs/demo/quietstep_demo.mp4) — 約 50 秒。quiet UX の空気感と、ホーム → クイズ → feedback → learner_type → result までの 1 セッション動線を収めた動画です。

GitHub の README プレビューではインライン再生されない場合があります。リンクからファイルを開いてください。
